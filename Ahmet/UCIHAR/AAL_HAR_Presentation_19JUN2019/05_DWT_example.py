...
DATA_FOLDER = 'C:/Users/winado/Desktop/Python/UCIHAR/'
...
def plot_fft_plus_power(ax, time, signal, plot_direction='horizontal', yticks=None, ylim=None, f_or_p='f'):  
    ...
    if plot_direction == 'horizontal':
        if f_or_p == 'f':
            ax.set_title('El Nino: FFT & PSD Values: By Frequency', fontsize=16)
            ax.set_xlabel('Frequency [Cycles/Year]', fontsize=12)
            ax.plot(f_values, fft_values, 'r-', label='Fourier Transform')
            ax.plot(f_values, fft_power, 'k--', linewidth=1, label='FFT Power Spectrum')
        elif f_or_p == 'p':
            ax.set_title('El Nino: FFT & PSD Values: By Period', fontsize=16)
